...
def evaluate_siamese(model, test_loader, device):
    ...
    plot_confusion_matrix(cm, title="Confusion Matrix (Siamese)")
...
def plot_error_rate_by_size_group(df):
    ...
    error_rates = grouped.apply(lambda x: 1 - x.mean()).reset_index(
        name="error_rate")
    error_rates["count"] = grouped.count().values

    plt.figure(figsize=(10, 5))
    ax = sns.barplot(data=error_rates, x="size_group", y="error_rate",
                     palette="viridis")
    for i, row in error_rates.iterrows():
        ax.text(i, row["error_rate"] + 0.02, f'n={row["count"]}', ha='center',
                fontsize=9)
...
def plot_error_gap_distribution_by_size_group(df):
    total_per_group = df.groupby("size_group", observed=True).size().rename(
        "total").reset_index()
    gap_counts = df.groupby(["size_group", "error_gap"],
                            observed=True).size().rename("count").reset_index()
    merged = gap_counts.merge(total_per_group, on="size_group")
    merged["percent"] = 100 * merged["count"] / merged["total"]

    plt.figure(figsize=(12, 6))
    ax = sns.lineplot(data=merged, x="size_group", y="percent",
                      hue="error_gap", marker='o', palette="coolwarm")
